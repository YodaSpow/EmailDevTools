  ...
  $(document).ready(function() {
    ...
    $(tog).click(function() {
      if (flag == 0) {
        ...
        // Works: document.body.innerHTML = document.body.innerHTML.replace(/(&lt;%).*(%&gt;)/g, '<!--EJS $& EJS-->');

        //$("body") = $("body").html.replaceAll(/(&lt;%).*(%&gt;)/g, '<!--EJS $& EJS-->');
        
        // CORS: $("body").html($("body").html().replace(/(&lt;%).*(%&gt;)/g, '<!--EJS $& EJS-->'));
        
        /* https://stackoverflow.com/questions/4886319/replace-text-in-html-page-with-jquery
          $('body').contents().filter(function() {
            return this.nodeType === 3;
          }).replaceWith(function() {
              return this.nodeValue.replace(/(&lt;%).*(%&gt;)/g, '<!--EJS $& EJS-->');
          });
          */
        
        /* Works and trigger CORS 
        var replaced = $("body").html().replace(new RegExp("(&lt;%).*(%&gt;)", "igm"),'The ALL new string');
        $("body").html(replaced);
        */
        ...
        */
        ...
        /* Works with Error breaks the tool CORS issue
        var replaced = $("body").html().replace(/(&lt;%).*(%&gt;)/g,'<!--EJS $& EJS-->');
        $("body").html(replaced);
        */
        ...
        //https://regexr.com/3ec7i
        //document.body.innerHTML = document.body.innerHTML.replace(/(<!--EJS)(.*?)(EJS-->)/igm, '$2'); 
        
        
        //document.body.innerHTML = document.body.innerHTML.replace(/(&lt;!--)/g, '');

        //document.body.innerHTML = document.body.innerHTML.replace('&lt;!--EJS','');
        //document.body.innerHTML = document.body.innerHTML.replace('EJS--&gt;','');
